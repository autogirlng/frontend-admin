...
const BookingSummaryPaymentLayout = () => {
  ...
  const fetchDetails = async () => {
    ...
    const bookingData = {
      ...
      bookingType:
        hoursBetweenISOStrings(startDate, endDate) > 1
  ...
  const createPaymentTicket = async () => {
    ...
    const vehicalAvailable = await http.get<IVehicleAvailable>(
      `/bookings/check-availability?vehicleId=${params.get(
        "selectedVehicleID"
      )}&startDate=${startDate}&endDate=${endDate}`
    );
    if (vehicalAvailable?.isAvailable) {
      const ticket = await http.post<PaymentData>(
        `/bookings/create/${params.get("selectedVehicleID")}`,
        bookingData
      );

      if (ticket) {
        setPaymentLink(
          `${process.env.NEXT_PUBLIC_USER_FRONTEND_URL}/review-booking?booking_id=${ticket.booking.id}`
        );
      }
    } else {
        ...
        <div className="flex flex-row justify-between">
          <div className="w-1/2 bg-[#f0f2f5]">
            <div className="p-4 text-gray-700">
              ...
              {loading ? (
                <div className="w-full bg-white flex p-3 flex-col items-center justify-center gap-y-2">
                  <Spinner />
                </div>
              ) : (
          ...
          <div className="w-1/2 flex items-center gap-y-2 justify-center flex-col">
            {loading ? (
              ...
              <>
                {" "}
                <p className="text-[27px] font-bold text-[#344054]">
                  This is The Last Step
                </p>
                <p className="text-sm text-[#667185]">Pay to confirm booking</p>
                <p className="font-bold">
                  Pay NGN {Number(bookingPrice?.totalPrice.toFixed(2)) || 0}
                </p>
                {paymentLink.length > 0 ? (
                  <div className="px-4 py-4 text-xs bg-[#edf8ff] rounded-full text-[#0673ff] w-[230px] flex items-center justify-between gap-x-2">
                  ...
                  <button
                    onClick={createPaymentTicket}
                    className="py-5 px-12 text-xs bg-[#0673ff] rounded-full text-white hover:shadow-md"
                  >
                    {creatingTicket ? (
