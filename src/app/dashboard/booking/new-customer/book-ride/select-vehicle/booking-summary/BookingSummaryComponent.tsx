...
const BookingSummary = () => {
  ...
  const [openTopLevelItem, setOpenTopLevelItem] = useState<string | null>(
    "vehicle-details"
  );
  const topLevelContentRefs: RefObject<Record<string, HTMLElement | null>> =
    useRef({});
  ...
  const fetchDetails = async () => {
    try {
      const data = await http.get<ListingInformation>(
        `/vehicle-onboarding/${params.get("selectedVehicleID")}`
      );
      const customer = await http.get<User>(
        `/user/admin/${params.get("customerID")}`
      );
      const startDate = new Date(
        `${params.get("pickupDate")}T${params.get("pickupTime")}`
      ).toISOString();
      const endDate = new Date(
        `${params.get("dropOffDate")}T${params.get("dropOffTime")}`
      ).toISOString();
      ...
      const bookingPrices = await http.post<BookingPricing>(
        "/bookings/calculate-price",
        { startDate, endDate, vehicleId: data?.id }
      );
      const images = [
          ...
          <div className="w-full max-w-4xl mx-auto p-4 rounded-xl font-inter">
                      ...
                      className={`flex-shrink-0 cursor-pointer border-3 rounded-lg overflow-hidden transition-all duration-300
                                                ${
                                                  index === currentIndex
                                                    ...
                                                    : "border-gray-300 opacity-70 hover:opacity-100 hover:border-blue-400"
                                                }
                                            `}
            ...
            <div className="flex flex-start flex-col">
              ...
              <div className="flex flex-wrap gap-x-1 gap-y-2 mt-3">
                <span className="bg-[#f0f2f5] rounded-md text-sm p-2 me-2">
                  Make: {vehicle?.make}
                </span>
                <span className="bg-[#f0f2f5] rounded-md text-sm p-2 me-2">
                  Model: {vehicle?.model}
                ...
                <span className="bg-[#f0f2f5] rounded-md text-sm p-2 me-2">
                  Seating Capacity: {vehicle?.numberOfSeats}
                </span>
              </div>
            </div>
            <div className="mt-2">
            ...
            <div className="flex flex-start flex-col mt-2">
              ...
              <div className="flex flex-wrap gap-x-1 gap-y-2 mt-3">
                ...
                <span className="bg-[#f0f2f5] rounded-md text-sm p-2 me-2">
                  20: ltrs Fuel Included
                </span>
                <span className="bg-[#f0f2f5] rounded-md text-sm p-2 me-2">
                  Free Cancellation <Link href="#">Learn More</Link>
                </span>
                <span className="bg-[#f0f2f5] rounded-md text-sm p-2 me-2">
                  Self Drive
                </span>
            ...
            <div className="flex flex-start flex-col mt-2">
              ...
              <div className="flex flex-wrap gap-x-1 gap-y-2 mt-3">
                {vehicle?.features?.map((feature, index) => (
                  <span
                    key={index}
                    className="bg-[#f0f2f5] rounded-md text-sm p-2 me-2"
                  >
                    {feature}
                  </span>
                ))}
            ...
            <AccordionItem id="hostDetails" title="Host Details">
              <div className="rounded-full px-1 flex flex-row border border-[#c3c8d0]">
                <div className="my-1 flex flex-row w-1/3 items-center border-r border-[#c3c8d0]">
                  <span className="p-5 bg-black text-white rounded-full ">
                    MJ
                  </span>
              ...
              <div className="bg-white rounded-lg mt-3 p-4">
                ...
                <p className="text-gray-700 text-sm leading-relaxed">
                  ...
                  Repellendus sequi veritatis nam asperiores illum odio
                  voluptates, unde dolores veniam iste amet. Illum accusantium
                  in distinctio necessitatibus quos fuga illo eum!
                </p>
              </div>
            </AccordionItem>
          </div>
    ...
    },
    {
      ...
      content: (
        <div className="p-4 bg-gray-50 rounded-lg">
            ...
            <button className="flex items-center px-4 py-1 ms-3 bg-[#f0f2f5] rounded-full text-sm text-gray-600 hover:bg-gray-100">
              ...
              >
                <path
                  ...
                  d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.452 3.551M9 12l6.452-3.551M12 21a9 9 0 110-18 9 9 0 010 18zm-4.684-2.658A3 3 0 1012 15h.001l6.452 3.551M12 9a3 3 0 100-6 3 3 0 000 6z"
                />
              </svg>
              Share
            </button>
          ...
          <AccordionItem id="prices" title="Prices">
            <div className="flex text-sm justify-start bg-white p-4">
              <div className="border-r border-[#e4e7ec] me-3 px-3">
                ...
                <p className="font-bold text-gray-900">
                  NGN {vehicle?.pricing?.dailyRate?.value}/day
                </p>
              </div>
              <div>
                <p className="text-sm text-gray-500">Extra hours</p>
                <p className="font-bold text-gray-900">
                  NGN {vehicle?.pricing?.extraHoursFee}/hr
                </p>
              </div>
            </div>
          </AccordionItem>

          <AccordionItem id="duration" title="Duration">
          ...
          <AccordionItem id="itinerary" title="Itinerary">
            <div className="p-4 bg-white text-sm">
              ...
              <div className="bg-white rounded-lg">
                <p className="text-gray-700 font-medium mb-2 mt-3">
                  Outskirt Locations
                </p>
          ...
          (tripPurpose && tripPurpose.length > 0) ? (
            <AccordionItem id="extras" title="Extra Details">
          ...
          <div className="bg-white ms-2 p-6 rounded-lg w-full mt-3 border border-[#e4e7ec] h-fit">
            ...
            <div className="space-y-4 border-t border-[#e4e7ec]">
              <div className="flex flex-row mt-3 justify-between">
                ...
                <p className="text-base text-gray-800 font-sm">
            ...
            <div className="space-y-4 ">
              <div className="flex flex-row justify-between">
                ...
                <p className="text-base text-gray-800 font-medium">
                  NGN {bookingPrice?.totalPrice}
                </p>
              </div>
              <div className="flex flex-row justify-between">
                <p className="text-sm text-gray-500">Extra hours</p>
                <p className="text-base text-[#98a2b3] font-sm">
                  NGN {bookingPrice?.breakdown.extraHoursFee}
                </p>
              </div>
              <div className="flex flex-row justify-between">
                <p className="text-sm text-gray-500">Discount </p>
                <p className="text-base text-gray-800 font-medium">
                  NGN {bookingPrice?.breakdown.discountAmount}
                </p>
              </div>
              ...
              <p className="text-xs text-[#0673ff]">
                Learn more about our free cancellation
              </p>
              <button className="w-full border text-sm border-[#101928] rounded-full px-3 py-2 hover:bg-black hover:text-white">
                Save Booking
              </button>
              <button
                onClick={() =>
                  router.push(
                    "/dashboard/booking/new-customer/book-ride/select-vehicle/booking-summary/payment-confirmation"
                  )
                }
                className="w-full text-white text-sm rounded-full px-3 py-2 bg-[#0673ff] hover:shadow-md"
              >
                Proceed Payment
              </button>
            </div>
