...
import { BannerResponse, CreateBannerPayload, GetBannerResponse } from "@/components/dashboard/announcement-banner/types";
...
export function useCreateAnnouncementBannerContent() {
    ...
    return useMutation<unknown, Error, CreateBannerPayload>({
        ...
        onSuccess: () => {
            ...
            queryClient.invalidateQueries({ queryKey: [ANNOUNCEMENT_BANNER_QUERY_KEY] });
        },
        onError: (error) => {
            toast.error(error.message || "Failed to create announcement banner content.");
        },
    });
}
...
export function useDeleteAnnouncementBannerContent(bannerId: string) {
    ...
    return useMutation<unknown, Error, void>({
        ...
        onError: (error) => {
            toast.error(error.message || "Failed to delete announcement banner content.");
        },
    });
}
